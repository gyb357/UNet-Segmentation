...
import torch
from utils import TernaryOperator
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        def inner_forward(x1, x2):
            x = self.trans(x1)
            x = torch.cat([x2, x], dim=1)
            return self.conv(x)
        return checkpoint(inner_forward, x1, x2)
...
class UNet(nn.Module):
    def __init__(self, in_channels: int, filter: int, out_channels: int, kernel_size: int, bias: bool, dropout: float):
